...
package com.daxode.odinbrains.zig.lsp;

import com.daxode.odinbrains.ZigBundle;
import com.daxode.odinbrains.common.util.ApplicationUtil;
import com.daxode.odinbrains.common.util.StringUtil;
import com.daxode.odinbrains.lsp.IntellijLanguageClient;
import com.daxode.odinbrains.lsp.utils.FileUtils;
import com.daxode.odinbrains.zig.environment.ZLSConfigProvider;
import com.daxode.odinbrains.zig.settings.ZLSProjectSettingsService;
...
public class ZLSStartupActivity implements ProjectActivity {
        ...
        ApplicationManager.getApplication().executeOnPooledThread(() -> {
            ...
            try {
                ...
                if (!configPath.isBlank() && !validatePath("ZLS Config", configPath, false)) {
                    Notifications.Bus.notify(new Notification("OdinBrains.ZLS", "Using default config path.",
                ...
                if (configPath == null || configPath.isBlank()) {
                    blk:
                    try {
                        val tmpFile = FileUtil.createTempFile("odinbrains-zls-autoconf", ".json", true).toPath();
                        ...
                        if (StringUtil.isEmpty(config.zig_exe_path()) && StringUtil.isEmpty(config.zig_lib_path())) {
                            ...
                            // Notifications.Bus.notify(new Notification("OdinBrains.ZLS", "(ZLS) Failed to detect zig path from project toolchain", NotificationType.WARNING));
                            configOK = false;
                            break blk;
                        }
                        ...
                        Notifications.Bus.notify(new Notification("OdinBrains.ZLS", "Failed to create automatic zls config file",
    ...
    private static boolean validatePath(String name, String pathTxt, boolean dir) {
        ...
        try {
            ...
            Notifications.Bus.notify(
                    new Notification("OdinBrains.ZLS", "No " + name, "Invalid " + name + " at path \"" + pathTxt + "\"",
                                     NotificationType.ERROR));
            return false;
        }
        if (!Files.exists(path)) {
            Notifications.Bus.notify(new Notification("OdinBrains.ZLS", "No " + name,
    ...
    public Object execute(@NotNull Project project, @NotNull Continuation<? super Unit> continuation) {
        ...
        if (zlsPath.isEmpty()) {
            Notifications.Bus.notify(new Notification("OdinBrains.ZLS", "No ZLS binary",
